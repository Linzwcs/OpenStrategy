...
def run(ctx, strategy_id: str, help_args: bool):
    ...
    base_conf = OmegaConf.create({
        "platform":
        OmegaConf.structured(PlatformArgs),
        "strategy":
        OmegaConf.structured(UserStrategyArgs)
    })
    ...
    try:
        ...
        console.print(
            "[dim]Hint: Arguments must be prefixed with 'platform.' or 'strategy.'[/dim]"
        )
    ...
    config['infra']['builder'][
        'type'] = platform_obj.backend  # 对应 PlatformArgs 中的 backend
...
def create(strategy_name: str):
    ...
    if os.path.exists(base_path):
        console.print(
            f"[bold red]Error:[/bold red] Directory '{strategy_name}' already exists."
        )
        sys.exit(1)

    console.print(
        f"[bold green]🚀 Initializing new strategy:[/bold green] {strategy_name}"
    )
